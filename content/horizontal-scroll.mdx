...
In the past, to fix these problems, I would have removed the padding of the wrapper and apply padding to `<h1/>` and `.row` one by one. However, I just learned another way of fixing these problems by only applying styles to the relevant element, which is `.row` class. Much easier, right?
